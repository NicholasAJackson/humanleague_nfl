...
- `/mock-draft` — Timed snake mock draft using **Sleeper Half-PPR ADP**. Locked-in keepers from `/api/keeper-finals` (K1 + ceremony second) fill cost-round slots from the last startup draft for **every** manager on the board. Before reveal / without finals, falls back to nominations (your keepers only for non-commissioners).
