...
- `/mock-draft` — Timed snake mock draft using **Sleeper Half-PPR ADP**. Locked-in keepers from `/api/keeper-finals` (K1 + ceremony second) fill cost-round slots from the last startup draft; select who you draft as to see **your** keepers named on the board (other teams show anonymous keeper markers). Before reveal / without finals, falls back to nominations (your keepers only for non-commissioners).
